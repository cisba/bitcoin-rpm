...
%define packaging_commit 3b394e8044cf67cd76aa47b84487aa2f3f3eed81

# avoid empty debug rpm (TBD: add debug tar)
%define debug_package %{nil}

# avoid stripping to allow easy checksum verify of binaries
...
Source11:       https://github.com/bitcoin-core/packaging/archive/%{packaging_commit}.tar.gz
Source12:       https://bitcoincore.org/bin/bitcoin-core-%{version}/bitcoin-%{version}-%{_arch}-linux-gnu.tar.gz

# https://github.com/bitcoin/bitcoin/issues/11632
#Patch1: 11632.diff

#Patch2: bitcoin-0.17.0-unicode.patch

# Don't run tests requiring network access, build env has network disabled
#Patch3: bitcoin-0.17.1-no-network-tests.patch

# Dest change address patch for Lamassu Bitcoin machine
#Patch99: bitcoin-0.17.0-destchange.patch
...
# Use other than `/tmp` as it usually doesn't have enough space in mock nspawn
# LD_LIBRARY_PATH=/opt/openssl-compat-bitcoin/lib PYTHONUNBUFFERED=1 test/functional/test_runner.py --tmpdir=/builddir/build/BUILD/tmp


%install
rm -rf %{buildroot}
mkdir %{buildroot}
cp share/examples/bitcoin.conf bitcoin.conf.example

# Install packaging files


#make INSTALL="install -p" CP="cp -p" DESTDIR=%{buildroot} install
tar --directory=%{buildroot} -xzf %{SOURCE12}
mv  %{buildroot}/bitcoin-%{version} %{buildroot}%{_prefix}
...
install -D -m644 -p packaging-%{packaging_commit}/debian/bitcoin-qt.desktop %{buildroot}%{_datadir}/applications/bitcoin-qt.desktop
desktop-file-validate %{buildroot}%{_datadir}/applications/bitcoin-qt.desktop
install -D -m644 -p packaging-%{packaging_commit}/debian/bitcoin-qt.protocol %{buildroot}%{_datadir}/kde4/services/bitcoin-qt.protocol
...
* Tue Dec 25 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.17.1-1
- Update to upstream 0.17.1

* Thu Nov  1 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.17.0.1-1
- Update to upstream 0.17.0.1
- Enable Wayland support on Fedora
- Workaround for upstream Qt bug on OpenSSL 1.1.1

* Fri Oct 26 2018 Michael Hampton <bitcoin@ringingliberyt.com> 0.17.0-2
- Disable tests which require non-localhost network

* Wed Oct 03 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.17.0-1
- Update to upstream 0.17.0

* Wed Sep 19 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.16.3-1
- Update to upstream 0.16.3

* Sun Jul 29 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.16.2-1
- Update to upstream 0.16.2

* Fri Jun 15 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.16.1-1
- Update to upstream 0.16.1

* Mon Feb 26 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.16.0-1
- Update to upstream 0.16.0

* Wed Feb 21 2018 Michael Hampton <bitcoin@ringingliberty.com> 0.15.1-2
- Disable broken test, ref bitcoin/bitcoin issue 11632

* Fri Nov 17 2017 Michael Hampton <bitcoin@ringingliberty.com> 0.15.1-1
- Update to upstream 0.15.1

* Tue Sep 19 2017 Michael Hampton <bitcoin@ringingliberty.com> 0.15.0.1-1
- Update to upstream 0.15.0.1

* Sat Sep 16 2017 Michael Hampton <bitcoin@ringingliberty.com> 0.15.0-1
- Update to upstream 0.15.0

* Sun Jun 18 2017 Michael Hampton <bitcoin@ringingliberty.com> 0.14.2-1
